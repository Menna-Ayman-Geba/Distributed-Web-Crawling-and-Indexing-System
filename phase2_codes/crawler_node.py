    ...
    for i, url in enumerate(urls):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                # Send both HTML and TXT mapping to Master
                comm.send({"s3_key": html_s3_key, "url": url}, dest=0, tag=2)
                logger.info(f"Sent mapping to Master: {html_s3_key} -> {url}")
                comm.send({"s3_key": txt_s3_key, "url": url}, dest=0, tag=2)
                logger.info(f"Sent mapping to Master: {txt_s3_key} -> {url}")
