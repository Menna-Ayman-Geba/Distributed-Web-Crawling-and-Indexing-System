...
def download_index_from_s3():
    """Download all index files from S3 and normalize filenames"""
    try:
        ...
        # Download all index files and rename them to remove indexer prefix
        for obj in response['Contents']:
            key = obj['Key']
            filename = os.path.basename(key)
            # Remove the indexer prefix (e.g., 'indexer0_', 'indexer1_') from filename
            normalized_filename = re.sub(r'^indexer\d+_', '', filename)
            if normalized_filename:
                local_path = os.path.join(INDEX_DIR, normalized_filename)
...
def simple_search():
    ...
    try:
        # Open the index directory with all downloaded files
        ix = open_dir(INDEX_DIR)
    except Exception as e:
        print(f"Error opening index: {e}")
        print("This might be due to incompatible index segments from multiple indexers.")
        return
            ...
            with ix.searcher() as searcher:
                ...
                if results:
                    ...
                    for i, hit in enumerate(results, 1):
                        s3_key = hit['title']  # This is the original S3 key from crawl_data/
                        url = None
                        # Try direct mapping first
                        if s3_key in url_mapping:
                            url = url_mapping[s3_key]
                        # If not found, search through the mapping for the s3_key
                        else:
                            for mapped_url, data in url_mapping.items():
                                if isinstance(data, dict) and 'text' in data and data['text'].get('s3_key') == s3_key:
                                    url = mapped_url
                                    break
                        url_display = url if url else f"URL not found in mapping for {s3_key}"
